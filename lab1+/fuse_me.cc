        ...
        struct fuse_file_info *fi)
{
    ...
    uint64_t end = _rdtsc();
    printf("fuseserver_write_cycle: %d", end - start);
#else
    fuse_reply_err(req, ENOSYS);
#endif
}
        ...
        mode_t mode, struct fuse_file_info *fi)
{
    ...
    uint64_t end = _rdtsc();
    printf("fuseserver_create_file_cycle: %d", end - start);
}
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
    ...
    if(parent == 2 && *name == 'n' && *(name+1) == '_' && *(name+2) == 'i'){
        // printf("-----------------------------cache fit-----------%d-----------------\n", dir0_cache_valid);
...
fuseserver_unlink(fuse_req_t req, fuse_ino_t parent, const char *name)
{
    ...
    uint64_t end = _rdtsc();
    printf("fuseserver_unlink_cycle: %d", end - start);
}
