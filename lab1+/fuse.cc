...
int myid;
yfs_client *yfs;
char root_d_entry[64];
struct fuse_entry_param root_attr_cache;
int dir0_cache_valid;
struct fuse_entry_param dir0_attr_cache;

// struct fuse_entry_param *reply_entries[1024];
// struct cache_node *directory_caches[1024];
...
{
#if 1
    // printf("\nfuseserver_write %016llx\n", ino);
    // uint64_t start = _rdtsc();
    if(dir0_cache_valid){
        fuse_reply_write(req, size);
        // uint64_t end = _rdtsc();
        // printf("\nfuseserver_write_cycle: %d", end - start);
        return;
    }
...
{
    // printf("\nfuseserver_lookup %016llx\n", parent);
    // int p = parent;
    // printf("\n\n--- %d ---\n\n", p);
    // uint64_t start = _rdtsc();
    if(parent == 1 && !strcmp(name, root_d_entry)){
        // printf("-----------------------------cache fit\n");
        fuse_reply_entry(req, &root_attr_cache);
        // uint64_t end = _rdtsc();
        // printf("\nfuseserver_lookup_cycle: %d", end - start);
        return;
    }
    if(parent == 2){
        // printf("-----------------------------cache fit-----------%d-----------------\n", dir0_cache_valid);
        if(dir0_cache_valid == 1){
            fuse_reply_entry(req, &dir0_attr_cache);
        }
        else{
            fuse_reply_err(req, ENOENT);
        }
    ...
    // uint64_t end = _rdtsc();
    // printf("fuseserver_lookup_cycle___1___: %d\n", end - start);
    ...
    if (found) {
        ...
        getattr(ino, e.attr);
        // end = _rdtsc();
        // printf("fuseserver_lookup_cycle___2___: %d", end - start);
        // start = _rdtsc();
        fuse_reply_entry(req, &e);
        // end = _rdtsc();
        // printf("fuseserver_lookup_cycle___3___: %d", end - start);
    } else {
        fuse_reply_err(req, ENOENT);
    }
    // uint64_t end = _rdtsc();
        ...
        mode_t mode)
{
    ...
    if (( ret = fuseserver_createhelper( parent, name, mode, &e, extent_protocol::T_DIR)) == yfs_client::OK) {
        if(parent == 1){
            ...
            memcpy(root_d_entry, name, length);
            root_attr_cache = e;
        }
        fuse_reply_entry(req, &e);    
    } else {
...
{
    // printf("\nfuseserver_unlink %016llx\n", parent);
    // uint64_t start = _rdtsc();
    if(dir0_cache_valid){
        dir0_cache_valid = 0;
        fuse_reply_err(req, 0);
        // uint64_t end = _rdtsc();
        // printf("\nfuseserver_unlink_cycle: %d", end - start);
        return;
    }
    int r;
